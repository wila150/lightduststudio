...
async function registerMedia({ filename, url, mediaType, originalName, folderId }) {
  try {
    await db.prepare(`
      INSERT INTO media (filename, url, media_type, original_name, folder_id)
      VALUES (?, ?, ?, ?, ?)
      ON CONFLICT(url) DO NOTHING
    `).run(filename, url, mediaType || 'image', originalName || '', folderId || null);
  } catch (err) {
...
async function isFileInUse(url) {
  const inProject = await db.prepare('SELECT 1 FROM portfolio_projects WHERE cover_url = ? OR video_url = ? LIMIT 1').get(url, url);
  if (inProject) return true;

  const inPhoto = await db.prepare('SELECT 1 FROM portfolio_photos WHERE url = ? LIMIT 1').get(url);
  if (inPhoto) return true;
  ...
  const inHomeBlocks = await db.prepare("SELECT 1 FROM home_blocks WHERE content LIKE '%' || ? || '%' LIMIT 1").get(url);
  if (inHomeBlocks) return true;
