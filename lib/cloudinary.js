...
function makeUpload(folder, { allowVideo = false, maxSizeMB = 25 } = {}) {
  ...
  return multer({
    ...
    fileFilter: (req, file, cb) => {
      // Busboy (multer's parser) decodes multipart field values as latin1
      // by default; non-ASCII filenames (Chinese, etc.) come through
      // mojibake'd unless re-decoded as the utf8 bytes they actually are.
